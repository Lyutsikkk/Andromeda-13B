...
/datum/admins/proc/manage_player_ranks()
	...
	var/choice = tgui_alert(usr, "Which rank would you like to manage?", "Manage Player Ranks", BLUEMOON_PLAYER_RANKS)
...
/datum/admins/proc/migrate_player_ranks()
	...
	var/choice = tgui_alert(usr, "Which rank would you like to migrate?", "Migrate Player Ranks", BLUEMOON_PLAYER_RANKS)
	if(!choice || !(choice in BLUEMOON_PLAYER_RANKS))
		return
